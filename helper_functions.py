...
class DatasetHandler:
    ...
    def __init__(self):
        ...
        root_path_dir = os.path.dirname(os.path.realpath(__file__))
        self.image_dir = os.path.join(root_path_dir, 'data\\rgb')
        self.depth_dir = os.path.join(root_path_dir, 'data\\depth')
        self.segmentation_dir = os.path.join(root_path_dir, 'data\\segmentation')
        ...
        # Define Segmentation of Colormap
        self.color_mappings = {'None': (0,0,0), #Black 
    ...
    def _read_image(self):
        image_name = self.image_dir + '\\' + str(self.current_frame) + '.png'
        self.image = cv2.imread(image_name)[:, :, ::-1]
    ...
    def _read_segmentation(self):
        segmentation_name = self.segmentation_dir + '\\' + str(self.current_frame) + '.dat'
        self.segmentation = np.loadtxt(segmentation_name, delimiter = ',')
    ...
    def vis_segmentation(self, segmented_image):
        ...
        colored_segmentation[segmented_image == 1] = self.color_mappings['Buildings']
        colored_segmentation[segmented_image == 4] = self.color_mappings['Pedestrians']
        colored_segmentation[segmented_image == 5] = self.color_mappings['Poles']
        colored_segmentation[segmented_image == 6] = self.color_mappings['Roadlines']
        colored_segmentation[segmented_image == 7] = self.color_mappings['Roads']
        colored_segmentation[segmented_image == 8] = self.color_mappings['Sidewalks']
        colored_segmentation[segmented_image == 10] = self.color_mappings['Vehicles']
    ...
    def vis_lanes(self, lane_lines):
        ...
        for line in lane_lines:
            ...
            image_out = cv2.line(image_out.astype(np.uint8), 
                                 (x1, y1), 
                                 (x2, y2), 
                                 (255, 0, 255), 
                                 7)
    ...
    def plot_free_space(self, segmentation):
        ...
        for x, z, seg in zip(xx.flatten('C'), depth.flatten('C'), segmentation.flatten('C')):
            if not(seg == 1):
